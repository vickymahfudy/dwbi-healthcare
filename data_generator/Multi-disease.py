...
print("Mulai proses generate data healthcare...")
...
# ==========================================
specialties = ['General Practice', 'Cardiology', 'Neurology', 'Internal Medicine', 'Surgery', 'Pediatrics', 'Orthopedics']
doctors = []
...
df_doctor = pd.DataFrame(doctors)

# ==========================================
# 3. GENERATE DIM_DIAGNOSIS (Berdasarkan kolom dataset asli)
# ==========================================
diagnosis_data = [
    ('E11', 'Type 2 Diabetes', 'DIABETES'),
    ('I10', 'Essential Hypertension', 'HYPERTENSION'),
    ('J45', 'Asthma', 'ASTHMA'),
    ('I20', 'Angina Pectoris', 'MI_CAD'),
    ('F32', 'Major Depressive Disorder', 'DEPRESSION'),
    ('N18', 'Chronic Kidney Disease', 'CHRONIC_KIDNEY'),
    ('K27', 'Peptic Ulcer', 'PEPTIC_ULCER'),
    ('Z00', 'General Medical Examination', 'GENERAL_CHECKUP')
]
df_diagnosis = pd.DataFrame(diagnosis_data, columns=['icd10_code', 'diagnosis_name', 'category'])

# ==========================================
# 4. GENERATE DIM_DEPARTMENT
# ==========================================
dept_data = [
    ('D01', 'Emergency Room'),
    ('D02', 'Outpatient Clinic'),
    ('D03', 'Inpatient Ward'),
    ('D04', 'Intensive Care Unit (ICU)'),
    ('D05', 'Surgery Room')
]
df_department = pd.DataFrame(dept_data, columns=['dept_id', 'dept_name'])

# ==========================================
# 5. GENERATE FACT_ENCOUNTER
# ==========================================
encounters = []
...
for i in range(1, NUM_ENCOUNTERS + 1):
    encounter_date = fake.date_between(start_date=start_date, end_date=end_date)
    los = random.randint(0, 14) # Length of stay (hari)
    
    # Perhitungan biaya dummy
    base_cost = random.uniform(500, 5000)
    ip_cost = base_cost * los if los > 0 else 0
    rx_cost = random.uniform(50, 1000)
    total_cost = ip_cost + rx_cost + (base_cost if los == 0 else 0)

    encounters.append({
        'encounter_id': f"TRX-{i:07d}",
        'patient_id': random.choice(df_patient['patient_id']),
        'doctor_id': random.choice(df_doctor['doctor_id']),
        'icd10_code': random.choice(df_diagnosis['icd10_code']),
        'dept_id': random.choice(df_department['dept_id']),
        'encounter_date': encounter_date.strftime('%Y-%m-%d'),
        'admission_type': random.choice(['Urgent', 'Elective', 'Emergency']),
        'length_of_stay': los,
        'medication_cost': round(rx_cost, 2),
        'inpatient_cost': round(ip_cost, 2),
...
# 6. EXPORT KE CSV
# ==========================================
os.makedirs('dwbi_data', exist_ok=True)
df_patient.to_csv('dwbi_data/dim_patient.csv', index=False)
df_doctor.to_csv('dwbi_data/dim_doctor.csv', index=False)
df_diagnosis.to_csv('dwbi_data/dim_diagnosis.csv', index=False)
df_department.to_csv('dwbi_data/dim_department.csv', index=False)
df_encounter.to_csv('dwbi_data/fact_encounter.csv', index=False)
print("Berhasil menyimpan 5 file CSV ke dalam folder 'dwbi_data'.")

# ==========================================
# 7. GENERATE SQL DDL FILE
# ==========================================
sql_ddl = """
-- ========================================================
...
with open('dwbi_data/schema_healthcare_dw.sql', 'w') as f:
    f.write(sql_ddl)
    
print("Berhasil membuat file SQL DDL: 'schema_healthcare_dw.sql'")
